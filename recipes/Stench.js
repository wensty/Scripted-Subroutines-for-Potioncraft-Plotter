...
const StrongStench = { Stench: 3 };
const Stench = { Stench: 2 };
const WeakStench = { Stench: 1 };
...
function r1() {
  ...
  derotateToAngle(191 * 0.36, { toAngle: false });
  logAddHeatVortex(Infinity);
  stirIntoVortex(3.1);
  heatAndPourToEdge(0.3, 4);
  logAddHeatVortex(4.02);
  stirToTurn({ preStir: 5.5 });
  const a = -123;
  straighten(degToRad(a), SaltNames.Sun, { maxGrains: 389 });
  ...
  straighten(degToRad(a), SaltNames.Sun, { maxGrains: 300, maxStir: 0.7 });
  ...
  straighten(degToRad(-115), SaltNames.Sun, { maxGrains: 130 });
  stirIntoVortex(5);
  ...
  logAddSunSalt(5);
  stirToTurn();
  straighten(degToRad(-112), SaltNames.Sun, { maxStir: 2.5, maxGrains: 353 });
  stirToDangerZoneExit();
  ...
  console.log(stirToTarget(Effects.Water.Stench, { preStir: 3.3, maxStir: 0.5 }).distance);
}
