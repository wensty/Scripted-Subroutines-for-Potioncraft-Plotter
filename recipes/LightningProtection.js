...
const recipes = {
  r1: {
    ...
    script: () => {
      ...
      straighten(getAngleOrigin() - degToRad(3), SaltType.Sun, { maxStir: 1.35 });
      ...
      straighten(degToRad(-167), SaltType.Moon, {
        preStir: 3.9,
        maxStir: 2.637,
        maxGrains: 88,
      });
      ...
      straighten(degToRad(170), SaltType.Moon, { maxGrains: m1 + m2 - getTotalMoon() });
      stirToDangerZoneExit();
      stirToZone({ overStir: false });
      straighten(degToRad(156.9), SaltType.Sun, {
  ...
  r2: {
    ...
    script: () => {
      ...
      logSkirt();
      logAddSunSalt(310);
      straighten(degToRad(180), SaltType.Sun, { preStir: 4.8, maxGrains: 125 });
      stirIntoVortex(2.6);
      console.log("~0:" + getAngleEntity());
      logAddHeatVortex(Infinity);
      stirIntoVortex(3.2);
      logAddHeatVortex(Infinity);
      straighten(degToRad(150), SaltType.Sun, { preStir: 9.6, maxGrains: 39 });
      stirIntoVortex(4.6);
      logAddSunSalt(501 - getTotalSun());
      derotateToAngle(189 * 0.36, { toAngle: false });
      logAddHeatVortex(Infinity);
      straighten(degToRad(-141.5), SaltType.Sun, { preStir: 5.522, ignoreReverse: false });
      stirToDangerZoneExit();
